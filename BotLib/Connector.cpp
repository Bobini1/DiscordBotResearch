...
{
    Connector::Connector()
        :intents(0), heartbeat_interval(0), readyReceived(false), targetNumGuilds(0), guildsCompleted(nullptr), token(L"")
    {
    ...
    std::chrono::milliseconds Connector::connect(std::wstring token, int intents)
    {
        this->token = token;
        ...
        catch (websocket_exception e) { LOG(ERROR) << e.what() << std::endl; terminate(); }
        ...
        return std::chrono::milliseconds(heartbeat_interval);
    }

    Connector::~Connector()
    {
        client->close().then([]() { LOG(INFO) << "Closed the connection." << std::endl; });
        delete client;
        delete webClient;
    }
    ...
    void Connector::hello(json::value d)
    {
        ...
        LOG(INFO) << "interval: " << heartbeat_interval << std::endl;
    ...
    json::value Connector::request(http::method method, std::wstring route, std::wstring query, json::value json)
    {
        ...
        request.set_request_uri(uri_builder(route).set_query(query).to_string());
        request.headers().add(L"Authorization", std::wstring(L"Bot ") + token);
    ...
    void Connector::identify() {
        ...
        json::value d;
        d[L"token"] = json::value::string(utility::conversions::to_utf16string(token));
        d[L"intents"] = intents;
